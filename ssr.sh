...
View_User_info(){
 ...
 协议和混淆后面的[ _compatible ]，指的是 兼容原版协议/混淆。"
	echo && echo "==================================================="
	echo "http://kistom.com/qr/index.html?q=${SSRurl}" | qrencode -o - -t UTF8
	
}
# 设置 配置信息
Set_config_user(){
	echo "请输入要设置的用户 用户名(请勿重复, 用于区分, 不支持中文、空格, 会报错 !)"
	read -e -p "(默认: mouren):" ssr_user
	[[ -z "${ssr_user}" ]] && ssr_user="mouren"
...
Set_config_port(){
	...
	read -e -p "(默认: 9527):" ssr_port
	[[ -z "$ssr_port" ]] && ssr_port="9527"
...
Set_config_password(){
	echo "请输入要设置的用户 密码"
	read -e -p "(默认: QQ505213941):" ssr_password
	[[ -z "${ssr_password}" ]] && ssr_password="QQ505213941"
